...
class Collect extends Base
{
    ...
    public function collect($id, $current = 0)
    {
        ...
        //
        $this->collect_list_set($id, $current);
    ...
    public function collect_thread($id)
    {
        ...
        if ($data === false) {
            $this->collect_rm($id);
            return $this->success('采集完成！', '/collect/index', ['state' => 'finish']);
        }
    ...
    public function test()
    {
        if ($this->request->isPost()) {
            ...
            foreach ($list_url as $key => $cont_url) {
                ...
                Gather::field_content($info, $cont_url, $this->request->param('test'));
                break;
            }
        } else {
    ...
    private function collect_list_get($id)
    {
        ...
        if (empty($list)){
            $this->error('完成采集','/collect/index', ['state' => 'finish']);
        }
